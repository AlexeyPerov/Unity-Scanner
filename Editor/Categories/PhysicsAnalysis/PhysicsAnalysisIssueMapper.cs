    ...
    public static class PhysicsAnalysisIssueMapper
    {
            ...
            PlatformProfile profile)
        {
            ...
            foreach (var data in results)
            {
                ...
                {
                    data.AddWarning("Scene has " + data.RigidbodyCount + " rigidbodies, threshold is " + profile.MaxRigidbodyCount + ".");
                ...
                if (settings.CheckStaticColliderOnMovingParent)
                {
                    foreach (var col in data.Colliders)
                    {
                        ...
                        {
                            data.AddWarning("Static collider '" + col.ObjectPath + "' has moving parent.");
                ...
                if (settings.CheckNoGravityNoConstraints)
                {
                    foreach (var rb in data.Rigidbodies)
                    {
                        ...
                        {
                            data.AddInfo("Non-kinematic Rigidbody '" + rb.ObjectPath + "' has no gravity and no constraints.");
                ...
                if (settings.CheckTriggerNonKinematic)
                {
                    foreach (var rb in data.Rigidbodies)
                    {
                        ...
                        {
                            data.AddInfo("Trigger Rigidbody '" + rb.ObjectPath + "' is not kinematic.");
                ...
                if (settings.CheckMeshColliderComplex)
                {
                    foreach (var col in data.Colliders)
                    {
                        ...
                        {
                            data.AddWarning("Mesh collider '" + col.ObjectPath + "' has " + col.TriangleCount + " triangles (threshold: " + profile.MaxMeshColliderTriangles + ").");
                ...
                if (settings.CheckConcaveMeshKinematic)
                {
                    foreach (var col in data.Colliders)
                    {
                        if (col.ColliderType == "MeshCollider" && !col.IsConvex)
                        {
                            ...
                            {
                                data.AddInfo("Concave mesh collider on non-kinematic body '" + col.ObjectPath + "'. Unity will auto-convert to convex.");
                ...
                if (settings.CheckMissingMaterial && data.RigidbodyCount > 0)
                {
                    foreach (var col in data.Colliders)
                    {
                        ...
                        {
                            data.AddInfo("Collider '" + col.ObjectPath + "' has no physics material.");
                ...
                if (settings.CheckLayerMatrixBloat)
                {
                    foreach (var pair in data.LayerCollisions)
                    {
                        ...
                        {
                            data.AddInfo("Layer " + LayerMask.LayerToName(pair.LayerA) + " (" + pair.LayerA + ") <-> " +
